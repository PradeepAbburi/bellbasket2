...
const BellJobDetail = () => {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      ...
      try {
        const q = query(
          collection(db, 'bell_job_applications'),
          where('jobId', '==', id),
        ...
        if (!snap.empty) {
          setHasApplied(true);
          setApplicationStatus(snap.docs[0].data().status || 'pending');
        }
      } catch (err) {
        console.error(err);
      }
      ...
      <div className="pt-24 px-4 max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          >
            ...
            <span className="text-xs font-black uppercase tracking-wider">Back to all jobs</span>
          </button>
                ...
                <div className="pt-6 border-t border-border/50 space-y-4">
                  ...
                  <div className="h-64 rounded-2xl overflow-hidden border border-border/50 relative z-0">
                    ...
                    <a 
                      ...
                      className="absolute top-3 right-3 z-10 bg-white dark:bg-zinc-900 border border-border/50 shadow-md text-foreground px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider hover:bg-slate-50 dark:hover:bg-zinc-800 transition-all flex items-center gap-1.5"
                    >
              ...
              if (status === 'reviewed') {
               ...
               return (
                <div className="text-center py-8 space-y-4">
                  ...
                  <div className="inline-block px-4 py-2 rounded-xl border text-xs font-black uppercase tracking-wider mb-2">
                    Status: <span className={statusColor.split(' ').slice(1).join(' ')}>{statusLabel}</span>
                  </div>
                  <p className="text-sm text-muted-foreground font-medium">The hiring team is currently processing your profile.</p>
                </div>
              );
            })() : job.status !== 'active' ? (
