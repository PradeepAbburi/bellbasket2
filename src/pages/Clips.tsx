...
import { ArrowLeft, Play, Sparkles, Heart, MessageCircle, Share2, ShoppingBag, Wrench, Send, X, ChevronUp, ChevronDown, CheckCircle2, User } from 'lucide-react';
...
const Clips = () => {
  ...
  const filteredClips = clips.filter(c => c.cardType === (activeTab === 'products' ? 'product' : 'service'));
  const currentClip = filteredClips[activeIdx] || filteredClips[0];

  const handleNextVideo = () => {
    if (activeIdx < filteredClips.length - 1) {
    ...
    <div className="min-h-screen bg-[#202020] text-white flex flex-col relative overflow-hidden">
      ...
      <main className="flex-1 w-full max-w-md mx-auto px-4 pt-24 pb-16 flex flex-col items-center justify-center relative z-10">
        ...
        <button
          ...
          className="self-start flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-amber-500 hover:text-amber-400 mb-6 transition-colors cursor-pointer"
        >
          <ArrowLeft className="w-4 h-4" /> Back to Browse
        </button>

        {/* Simulator View (Prototype Feed) */}
        <div className="w-full bg-zinc-950 border border-white/10 rounded-[2.5rem] aspect-[9/16] overflow-hidden relative flex flex-col justify-between shadow-2xl p-2.5 md:p-3">
          {/* Phone Camera notch */}
          ...
          {/* Category Tab Toggle inside Simulator */}
          <div className="absolute top-20 left-0 right-0 z-20 flex justify-center gap-1.5 px-4 pointer-events-auto">
            <button
              onClick={() => {
              ...
              className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider transition-all cursor-pointer ${
                activeTab === 'products'
                  ...
                  : 'bg-black/55 text-zinc-400 hover:text-white border border-white/5 backdrop-blur-md'
              }`}
            >
              Products
            </button>
          ...
          <div className="absolute left-6 top-1/2 -translate-y-1/2 z-20 flex flex-col gap-3">
            ...
            <button
              ...
              disabled={activeIdx === filteredClips.length - 1}
              className={`w-9 h-9 rounded-full flex items-center justify-center border transition-all ${
                activeIdx === filteredClips.length - 1
                  ? 'bg-zinc-900/30 border-white/5 text-zinc-600 cursor-not-allowed'
                  : 'bg-black/40 hover:bg-black/60 border-white/10 text-white cursor-pointer active:scale-90'
              }`}
            >
